...
with col1:
    unique_jobs = load_data("""
        WITH unique_listings AS (
            SELECT DISTINCT job_title, company 
            FROM JOB_LISTINGS_CLEAN
        )
        SELECT COUNT(*) as unique_count FROM unique_listings
    """)
    st.metric("📊 Unique Jobs", f"{unique_jobs['UNIQUE_COUNT'][0]:,}")
